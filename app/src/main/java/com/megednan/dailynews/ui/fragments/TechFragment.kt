...
import com.megednan.dailynews.databinding.TechFragmentBinding
import com.megednan.dailynews.viewmodels.BusinessViewModel
import com.megednan.dailynews.viewmodels.TechViewModel
import dagger.hilt.android.AndroidEntryPoint

@AndroidEntryPoint
class TechFragment : Fragment(R.layout.tech_fragment) {

    private val viewModel by viewModels<TechViewModel>()
    lateinit var newsAdapter: NewsAdapter
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)
        val binding= TechFragmentBinding.bind(view)
        val linearLayoutManager= LinearLayoutManager(context)
        newsAdapter= NewsAdapter()
        viewModel.getNews().observe(viewLifecycleOwner, Observer {
            newsAdapter.submitData(lifecycle, it)
        })
        binding.techFragRv.apply {
        ...
        newsAdapter.setOnItemClickListener {
            ...
            findNavController().navigate(
                    R.id.action_techFragment_to_articleFragment,
                    bundle
            )
